...
using namespace std;
  ...
  public:
    ...
    bool isHamiltonianCycle(vector<int>& path);
  ...
  bool Grafo::isHamiltonianCycle(vector<int>& path) {
    int n = path.size();
      for (int i = 0; i < n - 1; i++) {
          if (!existeAresta(path[i], path[i+1])) {
              return false;
          }
      }
      return existeAresta(path[n-1], path[0]);
  }

  bool Grafo::findHamiltonianCycle() {
    vector<int> vertices;
      for (int i = 0; i < this->numVertices; i++) {
      ...
      do {
          if (isHamiltonianCycle(vertices)) {
