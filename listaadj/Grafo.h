...
using namespace std;
  ...
  class encontrarUniao{
    private:
      vector<int> parent, rank;
    public:
      encontrarUniao(int n) : parent(n), rank(n,0){
        for(int i = 0; i < n; i++){
          parent[i]=i;
        }
      }
      ...
      void uniao(int u, int v){
        int rootU = encontrar(u);
        int rootV = encontrar(v);
        if(rootU != rootV){
  ...
  void Grafo::kruskal(){
    vector <Aresta> todasArestas;
    for (int v = 0; v < this->numVertices; v++){
      Aresta *adj = this->primeiroListaAdj(v);
      while (adj != NULL){
        todasArestas.push_back(*adj);
        delete adj;
        adj = this->proxAdj(v);
      } 
    }
    ...
    encontrarUniao uf (this->numVertices);
    vector <Aresta> AGM;
